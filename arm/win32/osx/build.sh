...
### BUILD PROCESS ###
status "BUILDING WINDOWS TO ARM CROSS COMPILER...."
check_prerequesites
...
make CFLAGS="${GDB_CFLAGS}" || abort "Failed to build '${GDB_PACKAGE}'"
[ -x "${CROSS_DIR}/bin/${TARGET_SYSTEM}-gdb" ] || make install || abort "Failed to install '${GDB_PACKAGE}'"
cd - > /dev/null

# all done
success "WINDOWS TO ARM CROSS COMPILER BUILT IN '${CROSS_DIR}'"
